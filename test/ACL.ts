...
import { events, REVERTED, TEST_ROLE } from "../utils/constants";
...
contract("ACL", (accounts: string[]) => {
  ...
  describe("hasRole", () => {
    it("superuser doesn't have the role", async () => {
      expect(await acl.hasRole(defaultAccount, TEST_ROLE)).to.be.false();
    });

    it("returns false when no role", async () => {
      expect(await acl.hasRole(accounts[1], TEST_ROLE)).to.be.false();
    });

    it("returns true when added to role", async () => {
      await acl.addRole(accounts[1], TEST_ROLE);

      expect(await acl.hasRole(accounts[1], TEST_ROLE)).to.be.true();
    });
  });
  ...
  describe("addRole", () => {
    ...
    it("fails without proper role", async () => {
      const addRole = acl.addRole(accounts[1], TEST_ROLE, { from: accounts[1] });
      await expect(addRole).to.eventually.be.rejectedWith(REVERTED);
    });

    it("works from TEST_ROLE", async () => {
      const SOME_ROLE = "some role, please ignore";
      expect(await acl.hasRole(accounts[2], TEST_ROLE)).to.be.false();

      await acl.addRole(accounts[1], TEST_ROLE);

      await expect(acl.addRole(accounts[2], SOME_ROLE)).to.eventually.be.fulfilled();

      expect(await acl.hasRole(accounts[2], SOME_ROLE)).to.be.true();
    });

    it("returns an event", async () => {
      const tx = await acl.addRole(accounts[1], TEST_ROLE);
      const event = findEvent(tx, events.ROLE_ADDED);

      expect(event).to.not.be.null();
      expect(event.args.granter).to.be.equal(defaultAccount);
      expect(event.args.grantee).to.be.equal(accounts[1]);
      expect(event.args.role).to.be.equal(TEST_ROLE);
    });

    it("doesn't crash when double adding", async () => {
      await acl.addRole(defaultAccount, TEST_ROLE);

      await expect(acl.addRole(defaultAccount, TEST_ROLE)).to.eventually.be.fulfilled();
    });
  });
  ...
  describe("removeRole", () => {
    it("removes a role", async () => {
      await acl.addRole(defaultAccount, TEST_ROLE);
      expect(await acl.hasRole(defaultAccount, TEST_ROLE)).to.be.true();

      await acl.removeRole(defaultAccount, TEST_ROLE);
    ...
    it("removes non-existen role", async () => {
      ...
      await expect(acl.removeRole(defaultAccount, TEST_ROLE)).to.eventually.be.fulfilled();
      expect(await acl.hasRole(defaultAccount, TEST_ROLE)).to.be.false();
    });
    ...
    it("doesn't work without TEST_ROLE access", async () => {
      ...
      const removeRole = acl.removeRole(defaultAccount, TEST_ROLE, { from: accounts[1]});
    ...
    it("works with TEST_ROLE access", async () => {
      ...
      await acl.removeRole(defaultAccount, TEST_ROLE, { from: accounts[1] });
    ...
    it("allows you to remove your own role", async () => {
      ...
      const removeRole = acl.removeRole(accounts[1], TEST_ROLE, { from: accounts[1] });
    ...
    it("allows to remove role somebody else added", async () => {
      ...
      await acl.addRole(accounts[2], TEST_ROLE);
    ...
    it("fires an event", async () => {
      ...
      const event = findEvent(tx, events.ROLE_REMOVED);
  ...
  describe("addSuperuser", () => {
    ...
    it("actually gives superpowers", async () => {
      const addFail = acl.addRole(accounts[1], TEST_ROLE, { from: accounts[1] });
      await expect(addFail).to.eventually.be.rejectedWith(REVERTED);

      await acl.addSuperuser(accounts[1]);

      const addSuccess = acl.addRole(accounts[1], TEST_ROLE, { from: accounts[1] });
    ...
    it("fires an event", async () => {
      ...
      const event = findEvent(tx, events.SUPERUSER_ADDDED);
  ...
  describe("removeSuperuser", () => {
    ...
    it("actually removes super powers", async () => {
      ...
      await expect(acl.addRole(defaultAccount, TEST_ROLE)).to.eventually.be.rejectedWith(REVERTED);
    });
    ...
    it("fires an event", async () => {
      ...
      const event = findEvent(tx, events.SUPERUSER_REMOVED);
